...
export default class Calculator {
  ...
  toggleInputButtons() {
    ...
    this.numberButtons.map(
      (numberButton) => (numberButton.disabled = maxDisplayHit)
    )

    const decimalRegex = new RegExp(/\./, "g")

    this.decimalButton.disabled =
      decimalRegex.test(this.currentNumber) || maxDisplayHit

    const minusRegex = new RegExp(/-/, "g")

    this.plusMinusButton.disabled =
      !maxDisplayHit || minusRegex.test(this.currentNumber)
        ? false
        : maxDisplayHit
  }
  ...
  input(value) {
    ...
    if (
      this.currentNumber.length !== this.maxNumberLength ||
      (value === "plus-minus" &&
        minusRegex.test(this.currentNumber) &&
        this.currentNumber.charAt(0) === "-")
    ) {
      const decimalRegex = new RegExp(/\./, "g")
   ...
   */
  display() {
    this.primaryDisplay.innerText = `${this.formatNumberWithCommas(
      this.currentNumber
    )}`
    ...
    this.secondaryDisplay.innerText =
      this.operator === ""
        ...
        : `${this.formatNumberWithCommas(this.firstNumber)} ${
            this.operator
          } ${this.formatNumberWithCommas(this.secondNumber)}`
  ...
  computedDisplay(early = false) {
    ...
    if (this.operator === String.fromCharCode(37)) {
      value = `${this.formatNumberWithCommas(this.firstNumber)}${
        this.operator
      } =`
    } else if (this.operator === String.fromCharCode(8730)) {
      value = `${this.operator}(${this.formatNumberWithCommas(
        this.firstNumber
      )}) =`
    } else {
      value = `${this.formatNumberWithCommas(this.firstNumber)} ${
  ...
  backspace() {
    if (this.currentNumber !== "0") {
      const slicedDisplay = this.currentNumber.slice(
        0,
        this.currentNumber.length - 1
      )

      this.currentNumber =
        slicedDisplay === "" || slicedDisplay === "-" ? "0" : slicedDisplay
